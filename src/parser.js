...
export class Parser {
  ...
  statement() {
    ...
    switch(token.type) {
        ...
        case "OPEN_BRACE": return this.block();
        case "WHILE": return this.while();
        case "RETURN":
          let value = null;
        ...
        case "UNREACHABLE": this.want("SEMICOLON"); return { type: "UNREACHABLE" };
        default:
          // TODO bad solution
          this.curr--;
  ...
  primary() {
    ...
    if (this.match("STRING", "INTEGER")) return this.literal(this.previous().literal)
  ...
  want(type) {
    ...
    if (token.type !== type) {
      throw new TaroError(`Expected token type ${type} got ${token.type}`, token.line)
    }
    return token
  }
